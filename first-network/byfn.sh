...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.pmc.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.pmc.com
    docker cp -a orderer.pmc.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.pmc.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.pmc.com

    for PEER in peer0.doctororg1.pmc.com peer1.doctororg1.pmc.com peer0.patientorg1.pmc.com peer1.patientorg1.pmc.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/doctororg1.pmc.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/patientorg1.pmc.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.doctororg1.pmc.com`` & ``peer0.patientorg1.pmc.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#####  Generating anchor peer update for DoctorOrg1MSP ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/DoctorOrg1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg DoctorOrg1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for DoctorOrg1MSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######  Generating anchor peer update for PatientOrg1MSP   #####"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/PatientOrg1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg PatientOrg1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for PatientOrg1MSP..."
    exit 1
  fi
  echo
}
